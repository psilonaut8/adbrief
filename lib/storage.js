...
// ── THUMBNAILS ────────────────────────────────────────────────────────────────
// Persisted copies of Meta creative images (CDN URLs expire) — keyed by
// `<client>__<adId or normalized adName>`, stored as base64.

async function saveThumb(key, contentType, base64Data) {
  const database = await connect();
  await database.collection('thumbs').updateOne(
    { key },
    { $set: { key, contentType, data: base64Data, updatedAt: new Date().toISOString() } },
    { upsert: true }
  );
}
