...
const COLUMN_ALIASES = {
  // Ad ID (Meta's unique identifier — used to fetch thumbnails from the API)
  'ad id': 'adId',
  'ad_id': 'adId',
  'adid': 'adId',
  ...
  // Results — the primary conversion count and what it's measuring
  'results': 'results',
  'result': 'results',
  'conversions': 'results',
  'result indicator': 'resultType',
  'result type': 'resultType',
  'conversion type': 'resultType',
  'primary metric': 'resultType',

  // Status / delivery — Meta exports include these separately
  'status': 'status',
  'ad status': 'status',
  'delivery': 'delivery',
...
function parseRows(rawRows, detectedCols) {
  ...
  for (let r = 1; r < rawRows.length; r++) {
    ...
    // Keep resultType as a string; clean up any trailing whitespace
    if (ad.resultType) ad.resultType = String(ad.resultType).trim();

    // Normalize numeric fields
    for (const field of ['spend', 'impressions', 'cpm', 'ctr', 'cpc', 'roas', 'frequency', 'reach', 'clicks', 'results']) {
    ...
    // Normalise status/delivery to a single adStatus field
    const rawStatus = String(ad.status || ad.delivery || '').trim();
    if (rawStatus) ad.adStatus = rawStatus;
    delete ad.status;
    delete ad.delivery;
