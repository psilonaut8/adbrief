...
function parseRows(rawRows) {
  ...
  for (let r = 1; r < rawRows.length; r++) {
    ...
    for (const field of ['dateStart', 'dateEnd', 'dateCreated']) {
      if (ad[field] != null) ad[field] = String(ad[field]).trim() || null;
    }
...
function parseBuffer(buffer, mimetype) {
  if (mimetype && /csv|text\/plain|tab-separated/.test(mimetype)) {
